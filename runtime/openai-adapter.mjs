...
function strictSchema(actionSchema) {
  ...
  return {
    type: "object",
    properties: {
      action: { type: "string", enum: actionSchema.oneOf.map((variant) => variant.properties.action.const) },
      arguments: { anyOf: actionSchema.oneOf.map((variant) => variant.properties.arguments) },
    },
...
export async function resolve(context, request) {
  ...
  try {
    ...
    clearTimeout(timeout); return failed(request, error?.name === "AbortError" ? "openai_timeout" : "openai_transport_failed");
  }
  clearTimeout(timeout);
  if (!response || response.status < 200 || response.status >= 300) return failed(request, `openai_http_${Number(response?.status) || 0}`);
  const declared = Number(response.headers?.get?.("content-length"));
  if (Number.isFinite(declared) && declared > maximumResponseBytes) return failed(request, "openai_response_too_large");
  const text = await response.text(); if (Buffer.byteLength(text, "utf8") > maximumResponseBytes) return failed(request, "openai_response_too_large");
  try {
    const parsed = exactResponse(JSON.parse(text), context.model);
    const action = JSON.parse(parsed.text); encodeAction(action);
  ...
  } catch (error) { return failed(request, String(error?.message ?? error).slice(0, 256)); }
}
...
export const _openAiInternals = { strictSchema, responsesRequest, exactResponse };
